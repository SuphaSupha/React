...
const App = () => {
  ...
  const names = ["Edvinas", "Antanas", "Jonas", "Gedas", "Greta"];
  return (
    <div className="container">
      <Custombtn>Edvinas</Custombtn>
      <Custombtn>Antanas</Custombtn>
      <Custombtn>Jonas</Custombtn>
      {names.map((name) => (
      ...
      {colors.map((color) => (
        <Hero
          title={color}
          subtitle={`Mano megstamiause spalva yra ${color}`}
          color={color}
        />
      ))}
      {/* <div className="center">
        <Custombtn>Paspausk mane</Custombtn>
        <Custombtn variant="contained">1000</Custombtn>
      </div>
