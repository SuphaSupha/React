...
import { LenguageModeContext } from "../../contexts/LenguageModeProv";
...
const NavigationBar = () => {
  ...
  const { isLenguageMode, modeToggle } = useContext(LenguageModeContext);

  return (
    <Container style={{ background: isNightMode ? "#3e2801" : "white" }}>
      <Logo>vetbee</Logo>
      <RightContainer>
        <nav>
        ...
        <div onClick={modeToggle}>{isLenguageMode ? "EN" : "LT"}</div>
      </RightContainer>
    </Container>
  );
};
